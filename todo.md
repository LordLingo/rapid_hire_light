...
- [ ] §146.2 — Refactor every consumer (Home Hero.tsx SPA pillars, Spa.tsx pillar grid, Shrm.tsx "what to expect" detail, Pricing.tsx hero subhead, ContactSuccess SHRM timeline) to import from `lib/spaMetrics.ts` instead of inlining the strings. Keep ARIA + testids untouched.
- [ ] §146.3 — Vitest `client/src/lib/spaMetrics.test.ts`: lock the canonical values, lock that every consumer file imports SPA_METRICS, and pin that no consumer file still hardcodes the numeric strings (so a future contributor can't drift one off).
...
- [ ] §147.1 — Add a pure `sortPosts(posts, sortKey)` helper to `client/src/lib/blogFilters.ts` supporting `"newest"` (default, by `date` desc), `"oldest"` (`date` asc), `"alphabetical"` (`title.toLocaleLowerCase()` asc), and `"depth"` (longest reading time first; ties broken by newest). All comparisons are non-mutating (return a new array) and deterministic.
- [ ] §147.2 — Extend `parseFiltersFromSearch` + `buildFiltersSearch` to round-trip a `sort` param (omit when default). Update the `parseInitialFilters` callsite in Blog.tsx accordingly.
- [ ] §147.3 — Add a shadcn `Select` next to `blog-search-input` (label sr-only, default "Newest first"). Stable testid `blog-sort-select`; each option carries a `data-value` matching the sort key. Wire to state, plumb through the filter pipeline as the final pass after listPosts → range → tag → query → sort.
- [ ] §147.4 — Extend `formatResultCount` to optionally append the sort label when sort ≠ default (e.g. "Showing 12 of 120 articles, sorted A–Z"). Lock new variants in the existing spec.
- [ ] §147.5 — Vitest in `client/src/lib/blogIndexFilters.test.ts`: 4 sort-key paths, default omission in `buildFiltersSearch`, round-trip with sort + tag + q, Blog.tsx source pins (Select testid, all 4 option `data-value`s, sortPosts call in the pipeline).
...
- [ ] §149.1 — `lib/blogFilters.ts`: export `BLOG_POSTS_PER_PAGE = 12`, plus pure helpers `paginatePosts(posts, page)` (1-indexed, clamps to valid range, returns `{ posts, page, totalPages, total }`) and `parsePageFromSearch(search)` / write-side `?page=` integration in the existing `buildFiltersSearch` / `parseFiltersFromSearch` pair. Page 1 is the default and is suppressed from the URL.
- [ ] §149.2 — `Blog.tsx`: feed `visiblePosts` (post-filter, post-sort) through `paginatePosts(visiblePosts, page)` and render only the page's slice. State for `page` initialized from URL, written back on change.
- [ ] §149.3 — `Blog.tsx`: render a pager below the grid with Prev / numbered window / Next, and an `aria-label="Blog pagination"` `<nav>` container. Numbered window collapses to first / current ±1 / last with ellipses when totalPages > 7 (`buildPagerWindow(page, totalPages)` pure helper). Stable testids: `blog-pager`, `blog-pager-prev`, `blog-pager-next`, `blog-pager-page` (one per page button), `blog-pager-page-active`, `blog-pager-ellipsis`.
- [ ] §149.4 — `Blog.tsx`: when filters/search/sort/tag change, reset `page` to 1 so a user filtering down from page 8 doesn't land on an empty page. Pager hides itself entirely when totalPages <= 1.
- [ ] §149.5 — `Blog.tsx`: clicking a page button (or Prev/Next) smoothly scrolls back to the top of the post grid (use the existing `#blog-grid` anchor or wire one) and updates `?page=`. Respects `prefers-reduced-motion` (instant jump when reduced motion is on).
- [ ] §149.6 — `Blog.tsx`: update `formatResultCount` call site to disclose the page range when relevant ("Showing 13-24 of 120 articles · page 2 of 10") via the existing `formatResultCount` helper (extend it to accept page + perPage).
...
- [ ] §149.8 — Run tsc + full vitest; webdev_save_checkpoint; deliver.

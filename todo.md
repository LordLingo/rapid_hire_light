...
## 175. Contact hero — replace synthetic call card with editorial photograph
- [x] Generated editorial-calm hero photograph (mid-30s rep with headset, warm cream/wood scene) and uploaded to CDN; canonical URL pinned in `CONTACT_HERO_URL` constant on Contact.tsx
- [x] Replaced `visual={<ContactCallCard />}` with framed `<img>` block (rounded-[18px] + paper-shadow + border) inside a `relative w-full` wrapper; opted into PageHero `visualBleed` so badge can overhang
- [x] Added `data-testid="contact-hero-badge-pickup"` floating chip at bottom-left, mirroring the dispute-rate badge pattern from /compliance; migrates the trust copy from the old call card (Live answer · M–F 7am–7pm CT / Average pickup 14 seconds / Direct extension · no IVR · phone)
...
- [x] §190 — Add vercel.json with catch-all rewrite to /index.html so SPA routes (e.g., /blog?tag=candidate-experience, /integrations, /resources/*) don't return Vercel's edge 404

## 190. Vercel deployment routing
- [x] Diagnosed: Vercel was serving `/` from index.html but every other route 404'd at the edge because no rewrite rule existed. Compounding issue: this is a full-stack Express+SPA project, so dynamic endpoints (/api/contact, /api/candidate-contact, /api/og/blog/*.svg, /blog/feed.xml, /blog/index.json) had no serverless-function handler at all.
- [x] Refactored server/index.ts: split startServer() into createApp({ apiOnly }) (returns the configured Express app without listening) + a now-guarded startServer() that only auto-runs when invoked as the entrypoint or under RAPID_HIRE_FORCE_LISTEN=1
- [x] Added api/index.ts as the Vercel serverless function entrypoint — imports createApp({ apiOnly: true }) and exports the Express instance as default. apiOnly=true skips the static middleware + SPA-fallback catch-all (those are handled by Vercel's edge per vercel.json rewrites)
- [x] Made the disk-write contact/candidate persistence best-effort: DATA_DIR routes to /tmp/rapid-hire-data on Vercel (writable per-invocation) vs project ./data elsewhere; both append helpers wrap their fs.writeFileSync in try/catch and return a boolean; both ensure*Store() helpers swallow EROFS/EPERM. POST handlers no longer return 500 on disk failure — always 201 once validation passes since the form's accepted regardless of whether the audit-trail JSON updated.
- [x] Added vercel.json with: explicit buildCommand `pnpm vite build && pnpm node scripts/prerender_top_posts.mjs`, outputDirectory `dist/public`, framework null (so Vercel doesn't auto-detect a stale framework), and 7 rewrites: /api/contact + /api/candidate-contact + /api/og/blog/:slug.svg + /api/og/blog/tag/:tag.svg + /blog/feed.xml + /blog/index.json all route to /api/index, then a SPA-fallback `/((?!api/|assets/|static/|.*\..*).*)` → /index.html that excludes API paths, asset bundles, the bundled /static/ images, and any path with a file extension (so Vercel's static-file serving handles those before the fallback)
- [x] Updated tsconfig.json to include the new api/ directory
...
## 194. Desktop header logo flex-shrink fix
- [x] Caught from a dev preview screenshot: the desktop header logo was rendering as ~12px-wide vertical stripes at certain viewport widths even though /static/rhs5-color-trimmed.png served 200 OK with intrinsic 1100×750. Root cause: the template's customized `.flex` rule sets `min-width: 0` on flex children so flex items can shrink past their content; with the desktop nav row competing for horizontal space (Resources dropdown + Sign in pill + Get-a-Quote pill + 8 nav links + gap-7 spacing), the flex algorithm collapsed the leftmost flex item — the Logo Link wrapper — past its intrinsic width, leaving the `<img>` with effectively zero horizontal room and turning the vertical 1100×750 lockup into vertical stripes.
- [x] Fix: added `shrink-0` to the desktop logo's Link className (`flex items-center gap-3` → `flex shrink-0 items-center gap-3`) so the flex algorithm refuses to shrink the Link past its content. Inline `§194` doc-comment explains why.
